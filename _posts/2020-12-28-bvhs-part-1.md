...
static inline struct bbox get_node_bbox(const struct node* node) {
    return (struct bbox) {
...
3. For each node `A`, find the nearby node `B` that minimizes the surface area of `A U B` within the search radius `R` (parameter of the algorithm).
   ...
   ```c
   static inline struct bbox bbox_union(struct bbox bbox1, struct bbox bbox2) {
       return (struct bbox) {
           .min = {
   ...
   static inline float bbox_half_area(struct bbox bbox) {
       float extents[] = {
...
5. Merge nodes that should be merged and loop back to step 3 if there is more than 1 node after merging.
...
Not only are those instructions slow on some Ryzen processors, but they do not have vector equivalents,
...
static inline uint64_t encode(uint64_t x, uint64_t y, uint64_t z, int log_bits) {
...
It is highly unlikely a memory pool implementation can beat that,
and it is also way easier to implement.
...
In the next posts, I want to show how to write a good traversal kernel, that is at the same time robust and fast. Stay tuned!
